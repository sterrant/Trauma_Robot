...
void forward() {     // move forward 
  digitalWrite(pinLB, HIGH);  // left track moves forward
  digitalWrite(pinRB, HIGH);  // right track moves forward
  analogWrite(pinLF,255);  
  analogWrite(pinRF,255);   
  }
void back() {          // move backward
  digitalWrite(pinLB, LOW);   // right track moves backward
  digitalWrite(pinRB, LOW);  // left track moves backward
  analogWrite(pinLF,255);   
  analogWrite(pinRF,255); 
  }
void stopp() {         // stop
